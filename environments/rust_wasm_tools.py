...
Tools (all local, air-gappable):
  rust_build_wasm : cargo build --target wasm32-unknown-unknown (or wasm-pack build)
                    -> produces a .wasm artifact; auditor checks it exists + size
  rust_run_wasm   : execute a .wasm via wasmtime if present on PATH; otherwise
                    reports capability (no runtime) instead of failing — the loop
                    stays bounded and honest.

The workstation (Victus) has the wasm32 target + wasm-pack 0.15.0. wasmtime is
NOT on PATH by default, so rust_run_wasm degrades gracefully.
"""
...
def build_wasm(crate_dir: str, use_pack: bool = False, release: bool = True) -> Dict[str, Any]:
    """Compile a Rust crate to wasm32-unknown-unknown. Returns a result dict.

    crate_dir: path to a cargo crate (must contain Cargo.toml)
    """
...
def run_wasm(wasm_path: str, args: Optional[list[str]] = None) -> Dict[str, Any]:
    """Run a .wasm via wasmtime if available; else report capability honestly."""
    wasm = Path(wasm_path)
    if not wasm.exists():
...
# A ready-made tool-set dict for LocalHostLoop
def toolset(crate_dir: str) -> Dict[str, Any]:
    """Return {tool_name: callable} for a LocalHostLoop, bound to `crate_dir`."""
    crate = Path(crate_dir)
